...
def update_chandelier_stop(
    ...
    if trail_min is None or trail_max is None:
        trail_min = config.config.get('trailing_stop_min_pct', 0.04)  # widened from 0.02
        trail_max = config.config.get('trailing_stop_max_pct', 0.07)  # widened from 0.04
    ...
    if side == 'long':
        ...
        if profit_pct < activation_threshold:
            return None, ''  # ✅ trade hasn't proved itself yet, don't trail

        last_close = df['close'].iloc[-1] if df is not None and not df.empty else current_price
        peak = max(position.get('peak_price', entry_price), last_close)
...
def evaluate_exit(
    ...
    # --- Layer 2: MACD Bar Exhaustion exit (min 8 candles on 4h = 32 hours) ---
    if df is not None and not df.empty:
        candles_held = position.get('candles_held', 0)
        logger.debug(f"[{symbol}] MACD exit check: candles_held={candles_held}")  # ✅ temporary debug
        should_exit, reason = check_macd_bar_exhaustion_exit(df, position)
        if should_exit:
            return True, reason

    # --- Layer 1: Signal reversal (min 6 candles on 4h = 24 hours) ---
    if df is not None and not df.empty:
        candles_held = position.get('candles_held', 0)
        if candles_held >= 6:  # ✅ raised from 3 to 6
